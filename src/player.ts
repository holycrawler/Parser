 ...
 * @see CoachReport
 */
function getCoachReports(tableEl: HTMLTableElement | null): CoachReport[] | null {
  if (!tableEl) return null;
  return [...(tableEl.querySelectorAll("tr[class*=row]") as NodeListOf<HTMLTableRowElement>)].map(
    (e: HTMLTableRowElement) => {
      const [coachNameEl, jptEl, starEl] = e.cells;
      const stars = getNumberOfStars(starEl);
      const [, coachName, reportsString] = coachNameEl.textContent!.trim().match(/^(.*?)\s*\((\d+)\s*[^\)]+\)$/)!;
      const numberOfReports = Number(reportsString);
      const jpt = jptEl.textContent!.trim() === "n/a" ? null : Number(jptEl.textContent!.trim());
      return {
        coach: coachName,
        numberOfReports,
        jpt,
 ...
 * @see ScoutReport
 */
function getScoutReports(scoutReportTable: HTMLTableElement | null): ScoutReport[] | null {
  if (!scoutReportTable) return null;
  return [...(scoutReportTable.querySelectorAll("tr[class*=row]") as NodeListOf<HTMLTableRowElement>)].map((e) => {
    const [scoutEl, starEl] = e.cells;
    const stars = getNumberOfStars(starEl);
    const [, scoutType, reportsString] = scoutEl.textContent!.trim().match(/^(.*?)\s*\((\d+)\s*[^\)]+\)$/)!;
    const numberOfReports = Number(reportsString);
    return {
      scout: scoutType,
      numberOfReports,
...
function parseTalentReport(talentEl: Element): TalentReport {
  ...
  const [table1, table2] = [...talentEl.querySelectorAll("#talentPanel table")] as HTMLTableElement[];
  ...
  const [scoutReportTable, coachReportTable] = isScout ? [table1, table2] : [table2, table1];
...
const parsePlayer = (doc: Document = document): Player => {
  ...
  const formHistoryString = (doc.querySelector("img[src*=form_history]") as HTMLImageElement).src!.match(
    /form_history=([\d-]+)/
  );
  const formHistory = formHistoryString ? formHistoryString[1].split("-").map(Number) : [];
  ...
  return {
    ...
    country: {
      code: (countryEl as HTMLImageElement).src.match(/(?<=flags_round\/half\/)\w+(?=\.png)/)![0],
    ...
    club: {
      ...
      country: {
        code: clubCountryEl.querySelector("img")!.src.match(/(?<=flags_small\/new\/)\w+(?=\.png)/)![0],
    ...
    formHistory,
    experience: Number((expEl.firstElementChild as HTMLImageElement).title.replace(/\D+/g, "")),
